...
所有成稿块都锁定时不发送模型请求。网页按实际 HTTP JSON 的 UTF-8 字节检查 2,000,000 字节上限，并检查完整模型输入的 1,800,000 字符上限；所有候选原稿仍完整保留，不自动裁剪。超限时先保存项目，再自行调整内容。

已有审阅与当前完整输入、保护规则、后端及配置完全一致时，网页提供“继续已有审阅”和“重新润色”选择。前者不发新请求；后者显式发起新的模型调用。编辑、来源或保护改变、配置变化、服务重启，以及过期审阅都不会直接复用。逐项接受仍走原有保护检查。

审阅中的“本次请求记录”保存输入签名、请求字节／字符数、耗时及后端返回的 usage。配置标识是不透明的进程内标识，不包含可见密钥；重启后失效。未返回 usage 时显示未知，不能据此当作零费用。没有新增模型配置变量，启动、登录和检查命令保持不变。
